...
proof -
  { fix rr :: real
    ...
    have "\<And>r ra. \<not> lgen False ra (1 - r\<^sup>2 / 2) \<or> lgen False ra (cos r)"
      using cos_lower_bound_0 by blast (* 0.0 ms *)
  ...
  then show "\<forall>r. \<bar>r::real\<bar> < 1 \<longrightarrow> \<bar>cos r\<bar> \<le> 1"
    by auto (* 20 ms *)
qed
(*The "show" can't be refined to the pending goal because the not(\<le>) in the tptp conjecture 
  is changed to < in the atp_proof, and hence in the isar proof*)
